...
namespace Stockfish
{
  ...
  {
    return PawnValue * (pos.count<PAWN>(c) - pos.count<PAWN>(~c)) + (pos.non_pawn_material(c) - pos.non_pawn_material(~c));
  }
  ...
  Value Eval::evaluate(const Position &pos)
  {
    ...
    bool lazy = abs(simpleEval) >= RookValue + KnightValue + 16 * shuffling * shuffling + abs(pos.this_thread()->bestValue) + abs(pos.this_thread()->rootSimpleEval);
    ...
    else
    {
      ...
      v = (nnue * (915 + npm + 9 * pos.count<PAWN>()) + optimism * (154 + npm + pos.count<PAWN>())) / 1024;
    }
