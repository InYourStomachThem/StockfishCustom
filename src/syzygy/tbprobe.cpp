...
namespace Stockfish
{
    ...
    namespace
    {
        ...
        {
            return wdl == WDLWin ? 1 : wdl == WDLCursedWin ? 101
        ...
        struct LR
        {
            ...
            Sym get()
            {
                return S == Left ? ((lr[1] & 0xF) << 8) | lr[0] : S == Right ? (lr[2] << 4) | (lr[1] >> 4)
                                                                             : (assert(false), Sym(-1));
            }
        };
        ...
        TBTable<WDL>::TBTable(const std::string &code) : TBTable()
        {
            ...
            bool c = !pos.count<PAWN>(BLACK) || (pos.count<PAWN>(WHITE) && pos.count<PAWN>(BLACK) >= pos.count<PAWN>(WHITE));
        ...
        Ret do_probe_table(const Position &pos, T *entry, WDLScore wdl, ProbeState *result)
        {
            ...
            if (entry->hasUniquePieces)
            {
                ...
                // First two pieces are on a1-h8 diagonal, third below
                    ...
                    idx = 6 * 63 * 62 + 4 * 28 * 62 + rank_of(squares[0]) * 7 * 28 + (rank_of(squares[1]) - adjust1) * 28 + MapB1H1H7[squares[2]];
        ...
        void set_groups(T &e, PairsData *d, int order[], File f)
        {
            ...
            int n = 0, firstLen = e.hasPawns ? 0 : e.hasUniquePieces ? 3
                                                                     : 2;
            d->groupLen[n] = 1;
        ...
        uint8_t *set_sizes(PairsData *d, uint8_t *data)
        {
            ...
            {
                d->base64[i] = (d->base64[i + 1] + number<Sym, LittleEndian>(&d->lowestSym[i]) - number<Sym, LittleEndian>(&d->lowestSym[i + 1])) / 2;
        ...
        void *mapped(TBTable<Type> &e, const Position &pos)
        {
            ...
            fname = (e.key == pos.material_key() ? w + 'v' + b : b + 'v' + w) + (Type == WDL ? ".rtbw" : ".rtbz");
